...
const MenuModule = ({}: MenuModuleProps) => {
	...
	const { Menu, createMenu, updateMenu, toggleMenu, deleteMenu } = useMenu();
	...
	const detailSubmitHandler = (data: any, e: any) => {
		...
		if (master.id == 'new') {
			updateMenu(master); // .then((response) => { /* response */ })

			closeDetailHandler();
		} else {
			createMenu(master); // .then((response) => { /* response */ })
	...
	const itemToggleHandler = (ids: (number | string)[]) => {
		...
		toggleMenu(master); // .then((response) => { /* response */ })
	};
	...
	const dialogConfirmHandler = () => {
		if (confirmDialogType == 'delete') {
			...
			deleteMenu(master); // .then((response) => { /* response */ })

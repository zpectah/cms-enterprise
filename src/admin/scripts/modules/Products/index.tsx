...
const ProductsModule = ({}: ProductsModuleProps) => {
	...
	const detailSubmitHandler = (data: any, e: any) => {
		...
		if (master.id == 'new') {
			updateProducts(master); // .then((response) => { /* response */ })

			closeDetailHandler();
		} else {
			createProducts(master); // .then((response) => { /* response */ })
	...
	const itemToggleHandler = (ids: (number | string)[]) => {
		...
		toggleProducts(master); // .then((response) => { /* response */ })
	};
	...
	const dialogConfirmHandler = () => {
		if (confirmDialogType == 'delete') {
			...
			deleteProducts(master); // .then((response) => { /* response */ })

...
const StoresModule = ({}: StoresModuleProps) => {
	...
	const openDetailHandler = (id: string, redirect?: boolean) => {
		setDetail(id);
		setDetailData(getDetailData(id, 'Stores', Stores));
	...
	const detailSubmitHandler = (data: any, e: any) => {
		...
		if (master.id == 'new') {
			updateStores(master); // .then((response) => { /* response */ })

			closeDetailHandler();
		} else {
			createStores(master); // .then((response) => { /* response */ })
	...
	const itemToggleHandler = (ids: (number | string)[]) => {
		...
		toggleStores(master); // .then((response) => { /* response */ })
	};
	...
	const dialogConfirmHandler = () => {
		if (confirmDialogType == 'delete') {
			...
			deleteStores(master); // .then((response) => { /* response */ })
